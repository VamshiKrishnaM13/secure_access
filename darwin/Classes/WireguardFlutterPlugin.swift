...
public class WireguardFlutterPlugin: NSObject, FlutterPlugin {
    ...
    private func getStats(result: @escaping FlutterResult) {
        WireguardFlutterPlugin.utils.getTransferData { downloadData, uploadData in
            guard let downloadData = downloadData, let uploadData = uploadData else {
                result(FlutterError(code: "NO_DATA", message: "Failed to retrieve transfer data", details: nil))
                return
            }
            let statsDict: [String: UInt64] = [
                "downloadBytes": downloadData,
                "uploadBytes": uploadData
            ]
            print("Plugin side stats: ", statsDict)
            result(statsDict)
        }
    }
  ...
  func stopVPN(completion: @escaping (Bool?) -> Void) {
    ...
    func getTransferData(completion: @escaping (UInt64?, UInt64?) -> Void) {
        NETunnelProviderManager.loadAllFromPreferences { managers, _ in
            ...
            do {
                let requestMessage = Data([UInt8(0)])
                try session.sendProviderMessage(requestMessage) { response in
                    guard let data = response, let decodedResponse = String(data: data, encoding: .utf8) else {
                        completion(nil, nil)
                        return
                    }
                    print("Debug: Decoded response: \(decodedResponse)")
                   let parsedResult = parseStats(decodedResponse)
                
                    let txBytes = UInt64(parsedResult["tx_bytes"] ?? "0") ?? 0
                    let rxBytes = UInt64(parsedResult["rx_bytes"] ?? "0") ?? 0
...
func parseStats(_ result: String) -> [String: String] {
    var stats: [String: String] = [:]
    
    result.split(separator: "\n").forEach { line in
        let pair = line.split(separator: "=", maxSplits: 1)
        if pair.count == 2 {
            stats[String(pair[0])] = String(pair[1])
        }
    }
